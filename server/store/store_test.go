...
func TestGetSpacesForTeam(t *testing.T) {
	...
	t.Run("returns every team space whose backing channel the user belongs to", func(t *testing.T) {
		spaces, err := s.GetSpacesForTeam(teamID, memberOfAll, store.SpaceVisibility{}, 0, 100)
		require.NoError(t, err)
		require.Len(t, spaces, 2)
	})

	t.Run("filters to the user's channel memberships", func(t *testing.T) {
		spaces, err := s.GetSpacesForTeam(teamID, memberOfOne, store.SpaceVisibility{}, 0, 100)
		require.NoError(t, err)
	...
	t.Run("user with no memberships gets an empty result", func(t *testing.T) {
		spaces, err := s.GetSpacesForTeam(teamID, mmmodel.NewId(), store.SpaceVisibility{}, 0, 100)
		require.NoError(t, err)
	...
	t.Run("former team member with a stale channel membership gets an empty result", func(t *testing.T) {
		...
		spaces, err := s.GetSpacesForTeam(teamID, formerMember, store.SpaceVisibility{}, 0, 100)
		require.NoError(t, err)
		require.Empty(t, spaces)
	})
	...
	t.Run("pagination excludes hidden spaces before offset/limit", func(t *testing.T) {
		...
		spaces, err := s.GetSpacesForTeam(teamID, memberOfOne, store.SpaceVisibility{}, 0, 10)
		require.NoError(t, err)
		require.Len(t, spaces, 1)
	})

	t.Run("rejects empty userID", func(t *testing.T) {
		_, err := s.GetSpacesForTeam(teamID, "", store.SpaceVisibility{}, 0, 100)
		require.Error(t, err)
	...
	t.Run("rejects non-positive limit", func(t *testing.T) {
		for _, limit := range []int{0, -1} {
			_, err := s.GetSpacesForTeam(teamID, memberOfAll, store.SpaceVisibility{}, 0, limit)
			require.Error(t, err)
			require.True(t, store.IsErrInvalidInput(err), "limit=%d must return ErrInvalidInput; got %v", limit, err)
		}
	})
	...
	t.Run("open non-member space is included only with the open fall-through, and never a private one", func(t *testing.T) {
		...
		withFallthrough, err := s.GetSpacesForTeam(teamID, nonMember, store.SpaceVisibility{HasOpenFallthrough: true}, 0, 100)
		require.NoError(t, err)
		var ids []string
		...
		withoutFallthrough, err := s.GetSpacesForTeam(teamID, nonMember, store.SpaceVisibility{}, 0, 100)
		require.NoError(t, err)
	...
	t.Run("callerSeesEverySpace returns the team's live spaces to a caller with no memberships", func(t *testing.T) {
		...
		filtered, err := s.GetSpacesForTeam(teamID, nonMember, store.SpaceVisibility{}, 0, 100)
		require.NoError(t, err)
		require.Empty(t, filtered, "without the bypass a caller with no memberships sees nothing")

		all, err := s.GetSpacesForTeam(teamID, nonMember, store.SpaceVisibility{SeesEverySpace: true}, 0, 100)
		require.NoError(t, err)
